...
type TCPServerTestSuite struct {
	...
	server      *TCPServer // reference to the TCP server instance
...
func (s *TCPServerTestSuite) TestEmptyDataField() {
	...
	testCases := []struct {
		...
		{
			name: "empty map",
			msg:  Message{Type: "test", Data: map[string]interface{}{}},
		},
		{
			name: "empty type",
			msg:  Message{Type: "", Data: map[string]interface{}{"key": "value"}},
		},
	}
